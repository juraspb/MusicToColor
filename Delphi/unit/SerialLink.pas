...
begin
  ...
  begin
    DoErrorEvent('Íå ìîãó îòêðûòü ïîðò');
    FError:= -1;
    Exit;
  end;
 try
  {Óñòàíîâèòü ïàðàìåòðû ïîðòà}
  if not GetCommState(FHandle, _DCB) then
  begin
    DoErrorEvent('Íå ìîãó ïîëó÷èòü ñîñòîÿíèå ïîðòà');
    FError:= -2;
    Exit;
  end;
  with _DCB do
  ...
  begin
    DoErrorEvent('Íå ìîãó óñòàíîâèòü ñîñòîÿíèå ïîðòà');
    FError:= -3;
    Exit;
  end;
...
//    ReadIntervalTimeout:= 20;
  ...
  begin
    DoErrorEvent('Íå ìîãó óñòàíîâèòü òàéìàóò ïîðòà');
    Exit;
  end;
  Result:= True;
end;
   ...
   begin
    for trynmb:= 1 to MAX_TRYNMB do
     begin
      ...
      begin
        DoErrorEvent('Íå ìîãó çàïèñàòü â ïîðò');
        FError:= -4;
        Exit;
      end;
     end;
...
//  PurgeComm(FHandle,PURGE_RXCLEAR);
  if not ReadFile(FHandle, RcvBuff, ToReceive, rcvd, nil) then
   begin
    DoErrorEvent('Íå ìîãó ïðî÷èòàòü èç ïîðòà');
    FError:= -5;
    Exit;
   end;
  if rcvd = 0 then
   begin
    DoErrorEvent('Ïóñòîé áóôåð ïîðòà');
    FError:= -6;
    Exit;
   end;
