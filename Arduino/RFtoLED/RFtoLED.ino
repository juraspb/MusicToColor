...
const uint32_t PROGMEM
  colorTab[]={
      ...
      0xFFFFFF,0xEFEFEF,0xDFDFDF,0xCFCFCF,0xBFBFBF,0xAFAFAF,0x9F9F9F,0x8F8F8F,0x7F7F7F,0x6F6F6F,0x5F5F5F,0x4F4F4F,0x3F3F3F,0x2F2F2F,0x1F1F1F,0x0F0F0F,0X000000}; // белый - чёрный
...
const TDWord PROGMEM progColor[] = {
  { 0,16,64,80},   // Цвета первой субпрограммы 1,2,3,4 цвета соответственно (для всех программ)
...
uint32_t clBlack; 
uint32_t clWhite; 
uint32_t rndNumber1 = 0; 
uint32_t rndNumber2 = 0; 
uint8_t nPhase = 0;
uint8_t bpData[20];
...
uint32_t bandMagn10(uint8_t n) {
  TColor cl;
  uint16_t gain;
  
  cl.dw = pgm_read_dword(&colorTab[96*n/(bandPass/2)]);
  gain=((readData[2*n]+readData[2*n+1])*brightness)>>9;
  cl = mulsGain(cl,gain);
  return strip.Color(cl.r, cl.g, cl.b);
}
...
void cmdMusical() {
      switch (subprog) {
         ...
         case 12: { zIBeam10(); break; }   // 244
         case 13: { zMidland10(); break; }
         case 14: { zCharm(); break; }
         case 15: { zRainbow10(); break; }
...
void loop(){
    ...
    if(radio.available()){        // Если в буфере имеются принятые данные, то получаем номер трубы, по которой они пришли, по ссылке на переменную pipe
      ...
      if (prog<29) {
        ...
        if (prog>0) {
          /* Программы использующие задержку */
          delay(param);
          switch (prog) {
            case 1: {
             switch (subprog) { 
          ...
          /* Программы не требующие задержки */
          switch (subprog) {
            case 0: { theaterChaseRainbow(); break; }
            case 1: { rainbowCycle(); break; }
